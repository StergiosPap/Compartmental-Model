...
class Person:
    ...
    def __init__(self, ID=None, houseID=None, agegroup=None, town=None):
        ...
        self.healthFactor = CreateHealthFactor(self)
        self.hygieneFactor = NormalDistributionFactor(mean=1.0, sd=0.7)
        self.legalityFactor = NormalDistributionFactor(mean=1.0, sd=0.7)
        
        self.routine = CreateRoutine(self)
        self.interactions = ExpectedDailyInteractions(self.routine)
        
def CreateRoutine(person):
    '''
    ...
    if (person.agegroup == "Underage"):
        schedule = CreateRoutineUnderage(person)
        
    elif (person.agegroup == "Young Adult"):
        schedule = CreateRoutineYoungAdult(person)
        
    elif (person.agegroup == "Adult"):
        schedule = CreateRoutineAdult(person)
        
    elif (person.agegroup == "Middle Aged"):
        schedule = CreateRoutineMiddleAged(person)
        
    else: #Elderly  
        schedule = CreateRoutineElderly(person)

    return schedule


def CreateRoutineUnderage(person):
    #Initialization
    routine = [None] * 24
...
def CreateRoutineYoungAdult(person):
    #Initialization
    routine = [None] * 24
    for i in range(24): #Default
        routine[i] = ['House', person.houseID]
    ...
    Transport(routine, person.town, 8, endtime+1)
...
def CreateRoutineAdult(person):
    ...
    Transport(routine, person.town, 8, 18)
    ...
    if (random.randint(1,10)<9): #80% chance for nighttime enternainment
        ...
        Transport(routine, person.town, starting_hour-1, starting_hour+3)
...
def CreateRoutineMiddleAged(person):
    #Initialization
    routine = [None] * 24
    for i in range(24): #Default
        routine[i] = ['House', person.houseID]
...
def CreateRoutineElderly(person):
    #Initialization
    routine = [None] * 24
    for i in range(24): #Default
        routine[i] = ['House', person.houseID]
    ...
    Transport(routine, person.town, starting_hour-1, starting_hour+4)
        
    return routine
    

    
def CreateHealthFactor(person):
    '''
...
def NormalDistributionFactor(mean, sd):
    '''
...
def Transport(routine, town, starttime, endtime):
...
def ExpectedDailyInteractions(routine):
    '''
